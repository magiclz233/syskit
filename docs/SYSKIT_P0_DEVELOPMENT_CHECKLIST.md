...
| 顺序 | ID | 状态 | 需求项 | 关键交付 |
|---|---|---|---|---|
| 1 | `P0-001` | `待开发` | 建立新 CLI 工程骨架 | `main.go` 仅做启动；引入 `cmd/` 根命令；子命令注册结构落地 |
| 2 | `P0-002` | `待开发` | 实现全局参数与统一退出码基线 | 落地 `--format`、`--json`、`--output`、`--config`、`--policy`、`--quiet`、`--verbose`、`--dry-run`、`--apply`、`--yes`、`--fail-on`、`--timeout`、`--no-color` |
...
1. 是否接受本清单作为 P0 唯一开发顺序文档。
2. 是否接受“默认按本文档顺序逐项开发，不随意跳项”的规则。
3. 项目名称是否确定使用 `syskit`，还是保留/过渡为其他名称。
