...
命令树中保留 `file` 等占位命令，并按开发清单逐项替换为正式实现（例如已落地 `port ping/scan`、`net conn/listen/speed`、`dns resolve/bench`、`ping`、`traceroute`、`cpu burst/watch`、`mem leak/watch`、`monitor all`、`service list/check`、`doctor network`、`doctor disk-full`、`doctor slowness`）。
